      ...
      <div style={{background:C.bg3,border:`1px solid ${C.borderD}`,borderRadius:10,padding:24,
        ...
        <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
          ...
          <button onClick={onConfirm} style={{padding:'7px 16px',borderRadius:6,border:`1px solid ${C.accent}`,
            background:C.accent,color:'#fff',fontSize:13,cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontWeight:500}}>Replace</button>
        </div>
      </div>
    </div>
  )
            ...
            {visibleQuals.map((qual,i)=>{
              ...
              return (
                <div key={qual.id} style={{height:BLOCK_H,display:'flex',borderBottom:`2px solid ${C.borderL}`,background:qi%2===0?C.bg3:C.bg}}>
                  <div style={{width:100,minWidth:100,display:'flex',flexDirection:'column',justifyContent:'center',
                    padding:'4px 8px',borderRight:`1px solid ${C.border}`,gap:4}}>
                    <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:C.accent,lineHeight:1.2}}>{qual.id}</span>
                    <span style={{fontSize:11,color:C.text,lineHeight:1.3,wordBreak:'break-word',
...
function MatrixView({data,updateData}) {
  ...
  const handleToggle=useCallback((qualId,level,jobId)=>{
    ...
    if(existing===level) {
      updateData(d=>{ const n={...d.assignments}; delete n[`${qualId}_${level}_${jobId}`]; return{...d,assignments:n} })
  ...
  function doReplace() {
    ...
    updateData(d=>{
      ...
      n[`${qualId}_${newLevel}_${jobId}`]=true
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden',background:C.bg}}>
      {confirm&&<ConfirmDialog
        message={`Replace Level ${confirm.oldLevel} with Level ${confirm.newLevel} for:\n"${confirmQualName}"?`}
